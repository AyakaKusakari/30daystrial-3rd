...
wp_enqueue_style('my', get_template_directory_uri() . '/html/css/style.css', array(), '1.0.0', 'all');
wp_enqueue_script('my', get_template_directory_uri() . '/html/js/script.js', array( 'jquery' ), '1.0.0', true);
//sns.jsを追記
wp_enqueue_script('sns', get_template_directory_uri() . '/html/js/sns.js', array( 'jquery' ), '1.0.0', true);
//highlight.jsを追記
wp_enqueue_style('highlightjs', '//cdnjs.cloudflare.com/ajax/libs/highlight.js/9.15.8/styles/default.min.css', array(), '9.15.8');
wp_enqueue_script('highlightjs', '//cdnjs.cloudflare.com/ajax/libs/highlight.js/9.15.8/highlight.min.js', array(), '9.15.8', true);
}
...
function my_get_post_tags( $id = 0 ) {
    ...
    if ( $this_tags ) {
        foreach( $this_tags as $this_tag ){
            echo '<div class="entry-tag-item"><a href="'. esc_url( get_tag_link($this_tag->term_id) ) .'">'. esc_html( $this_tag->name ) .'</a></div><!-- /entry-tag-item -->';
        }
    }
}
...
add_action('we_footer', 'my_highlight', 30);

/**
* ハイライトのショートコード
*/
function my_highlights ($atts, $content = '') {
    $atts = shortcode_atts(array(
        'class' => html,
    ), $atts);
    extract($atts);
    return '<pre><code class=" . $class . "></code></pre>';
}
add_shortcode('highlight', 'my_highlights');